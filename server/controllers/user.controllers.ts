...
export const postUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const newUser = UserService.createUser({...sanitizedInput, username: sanitizedInput.username.toLowerCase(), isAdmin: req.body.isAdmin});
    await UserService.setUserPassword(newUser, newUser.password);
    try {
      await UserService.saveUser(newUser);
      return res.status(200).send({ message: "A new user has been created" });
    } catch (error) {
